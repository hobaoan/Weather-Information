    ...
    func setupBarChartView() {
        ...
        for (index, weatherInfo) in weatherForecast.list.enumerated() {
            ...
            if !uniqueDtTextSet.contains(String(dtText)) { // Step 2: Check if dtText is not in the set
                ...
                let entry = BarChartDataEntry(x: Double(dtTexts.count - 1), y: temp) // Step 3: Use the updated count for x-axis index
                entries.append(entry)
            }
        }
        
        let dataSet = BarChartDataSet(entries: entries, label: "Temperature")
